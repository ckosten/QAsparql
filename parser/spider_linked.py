...
from kb.dbpedia_spider import DBpedia
from parser.answerparser import AnswerParser


class Spider_Linked:
    def __init__(self, path="./data/SPIDER/linked.json"):
        self.raw_data = []
        self.qapairs = []
        self.path = path
        self.parser = Spider_LinkedParser()

    def load(self):
        with open(self.path) as data_file:
            self.raw_data = json.load(data_file)
#here is where the AnswerSet is coming from
    def parse(self):
...
class Spider_LinkedParser(AnswerParser):
    def __init__(self):
        super(Spider_LinkedParser, self).__init__(DBpedia(one_hop_bloom_file="./data/blooms/spo1.bloom"))
    ...
    def parse_sparql(self, raw_query):
        ...
        uris = URIs([Uri(raw_uri, self.kb.parse_uri) for raw_uri in re.findall('(<[^>]*>|\?[^ ]*|dbo\:[^ ]*)', raw_query)])
