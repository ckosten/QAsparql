...
from parser.spider_linked import Spider_LinkedParser
...
def query_parse(filepath):
    ...
    with open(filepath) as datafile, \
        ...
        for clause in tqdm(datafile):
            #clauses = line.split(" .")
            #this was needed for when there are multiple triples in a generated query
            vars = dict()
            root = None
...
if __name__ == '__main__':
    ...
    train_dir = os.path.join(spider_dir, 'train')
    dev_dir = os.path.join(spider_dir, 'dev')
    test_dir = os.path.join(spider_dir, 'test')
    make_dirs([train_dir, dev_dir, test_dir])

    # split into separate files
    train_filepath = os.path.join(spider_dir, 'spider_train.json')
    trail_filepath = os.path.join(spider_dir, 'spider_trial.json')
    test_filepath = os.path.join(spider_dir, 'spider_test.json')

    ds = json.load(open("output/spider_gold_gen_queries_test.json"))
    ...
    # get vocabulary
    build_vocab(
        glob.glob(os.path.join(spider_dir, '*/*.toks')),
        os.path.join(spider_dir, 'vocab.txt'))
    build_vocab(
        glob.glob(os.path.join(spider_dir, '*/*.toks')),
        os.path.join(spider_dir, 'vocab-cased.txt'),
        lowercase=False)
